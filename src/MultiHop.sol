...
contract Liquidator {
    ...
    struct SwapStep {
        address target;   // adresse du DEX
        bytes   data;     // calldata encodé off-chain
        address tokenIn;  // token approuvé avant le call
        address tokenOut; // token récupéré après le call
    }

    struct LiquidationData {
        MarketParams marketParams;
        SwapStep[]   steps;   // pipeline ordonné, longueur >= 1
        uint256      minOut;  // garde-fou global sur le output final
    }
    ...
    /// @notice Autorise ou révoque un DEX/router pouvant être appelé dans le pipeline.
    function setTarget(address target, bool allowed) external onlyOwner {
        allowedTargets[target] = allowed;
    }

    /// @notice Récupère tous les tokens résiduels vers le owner.
    ...
    /// @notice Déclenche une liquidation Morpho avec un pipeline de swaps multi-DEX.
    /// @param marketParams  Paramètres du marché Morpho.
    /// @param borrower      Adresse de l'emprunteur à liquider.
    /// @param seizedAssets  Montant de collatéral à saisir (0 si repaidShares spécifié).
    /// @param repaidShares  Shares de dette à rembourser (0 si seizedAssets spécifié).
    /// @param steps         Pipeline ordonné de swaps : chaque step est un DEX distinct.
    /// @param minOut        Montant minimum de loanToken attendu après le pipeline complet.
    function liquidate(
        ...
        // Pré-approuver Morpho pour le remboursement de la dette
    ...
    /// @notice Callback appelé par Morpho pendant la liquidation.
    ///         Exécute le pipeline de swaps et vérifie que le output couvre la dette.
    function onMorphoLiquidate(uint256 repaidAssets, bytes calldata data) external onlyMorpho {
        ...
        for (uint256 i = 0; i < len; i++) {
            ...
            // Reset allowance résiduelle (slippage, fee-on-transfer…)
    ...
    /// @dev Revert en propageant l'erreur du DEX tout en indiquant le step fautif.
    ///      Si le DEX n'a retourné aucune raison, revert avec StepCallFailed.
